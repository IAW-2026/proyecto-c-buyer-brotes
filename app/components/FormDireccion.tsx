...
export default function FormDireccion({ direccionActual }: Props) {
  const [valor, setValor] = useState(direccionActual ?? '')
  ...
  const guardar = async () => {
    if (!valor.trim()) {
      setMensaje({ tipo: 'error', texto: 'La dirección no puede estar vacía' })
      return
    }
    ...
    try {
      ...
      setMensaje({ tipo: 'ok', texto: '¡Dirección guardada! Ya podés realizar compras.' })
      setEditando(false)
    } catch {
      ...
      {editando ? (
        <>
          ...
          <div className="flex gap-2">
            ...
            {direccionActual && (
              <button
                onClick={() => { setEditando(false); setValor(direccionActual); setMensaje(null) }}
